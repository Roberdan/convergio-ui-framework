...
const SIZE_PX = { sm: 120, md: 180, lg: 240 } as const; type GaugeSize = keyof typeof SIZE_PX | "fluid"
...
function render(
  ...
  if (od) {
    ...
    for (let i = 0; i < od.digits.length; i++) {
      ...
      ctx.fillStyle = last ? "#DC0000" : pal.odometerBg; ctx.strokeStyle = last ? "#DC0000" : pal.odometerBorder; ctx.lineWidth = 0.8
...
function MnGauge({
  ...
  React.useEffect(() => {
    ...
    const go = (p: number) => render(cvs.current!, pal, s, p, value, min, max, startAngle, endAngle, ticks, subticks, numbers, color, unit, label, arcBar, subDials, innerRing, odometer, statusLed, trend)
  ...
  React.useEffect(() => {
    ...
    const ro = new ResizeObserver(() => { clearTimeout(tid); tid = setTimeout(() => { const s = px(); if (s > 0 && cvs.current && wrap.current) render(cvs.current, readPalette(wrap.current), s, 1, value, min, max, startAngle, endAngle, ticks, subticks, numbers, color, unit, label, arcBar, subDials, innerRing, odometer, statusLed, trend) }, 150) })
